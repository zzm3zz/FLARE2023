...
class SE_Conv_Block(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, flag=0, downsample=None, drop_out=False):
        ...
        elif planes == 480 and flag == 1:
            self.globalAvgPool = nn.AvgPool2d((16, 16), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((16, 16), stride=1)
        elif planes == 480 and flag == 2:
            self.globalAvgPool = nn.AvgPool2d((32, 32), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((32, 32), stride=1)
        elif planes == 480 and flag == 3:
            self.globalAvgPool = nn.AvgPool2d((64, 64), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((64, 64), stride=1)
        elif planes == 32:
            self.globalAvgPool = nn.AvgPool2d((512, 512), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((512, 512), stride=1)
        elif planes == 384:
            self.globalAvgPool = nn.AvgPool2d((128, 128), stride=1)    # (14, 18) for ISIC2018
            self.globalMaxPool = nn.MaxPool2d((128, 128), stride=1)
        elif planes == 192:
    ...
    def forward(self, x):
        ...
        # 256->128  128->64 64->32 32->16 + BN + ReLU
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
...
class channel_attention_block(nn.Module):
    ...
    def __init__(self, inplanes, stride=1, downsample=None, drop_out=False, size=56):
        super(channel_attention_block, self).__init__()
        self.conv1 = conv3x3(inplanes, 128, stride)
        self.bn1 = nn.BatchNorm2d(128)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(128, 256, stride)
        ...
        self.globalAvgPool = nn.AvgPool2d((self.size, self.size), stride=1)  # (224, 300) for ISIC2018

        self.fc1 = nn.Linear(in_features=256, out_features=round(256))
        self.fc2 = nn.Linear(in_features=round(256), out_features=inplanes)
        self.sigmoid = nn.Sigmoid()
