...
from nnunet.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper, ImageCropper_test
...
def resample_patient_test(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
...
#     seg_torch = torch.from_numpy(seg).to(torch.float32)
#     seg_torch = torch.unsqueeze(seg_torch, 0)
...
#     reshaped_final_data = torch.squeeze(reshaped_final_data, 0)
#     reshaped_final_data = reshaped_final_data.numpy()
#     reshaped_final_seg = F.interpolate(seg_torch, size=new_size, mode='trilinear', align_corners=False)
#     reshaped_final_seg = torch.squeeze(reshaped_final_seg, 0)
#     reshaped_final_seg = reshaped_final_seg.numpy()
    
#     return reshaped_final_data, reshaped_final_seg
    # if force_separate_z is not None:
    #     do_separate_z = force_separate_z
    #     if force_separate_z:
    #         axis = get_lowres_axis(original_spacing)
...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize_test(self, data, target_spacing, properties, seg=None, force_separate_z=None):
        """
        data and seg must already have been transposed by transpose_forward. properties are the un-transposed values
        (spacing etc)
        :param data:
        ...
        # data[np.isnan(data)] = 0
        start_resample = time.time()
        # original_spacing = np.array(original_spacing_transposed)
        # max_size = 189959211
        # data_size = (data.shape[1]*original_spacing[0]/target_spacing[0]) * (data.shape[2]*original_spacing[1]/target_spacing[1]) * (data.shape[3]*original_spacing[2]/target_spacing[2])
        # print(data_size)
        # if data_size > max_size:
        #     target_spacing[0] = 1.5
        # else:
        #     target_spacing[0] = 1 
        data = resample_patient_test(data, seg, np.array(original_spacing_transposed), target_spacing,
                                     self.resample_order_data, self.resample_order_seg,
                                     force_separate_z=force_separate_z, order_z_data=0, order_z_seg=0,
                                     separate_z_anisotropy_threshold=self.resample_separate_z_anisotropy_threshold)
        print('resample time: ', time.time() - start_resample)
    ...
    def preprocess_test_case(self, data_files, target_spacing, seg_file=None, force_separate_z=True): # 改为 True
        data, properties = ImageCropper_test.crop_from_list_of_files_test(data_files, seg_file)
        data = data.transpose((0, *[i + 1 for i in self.transpose_forward]))
        # seg = seg.transpose((0, *[i + 1 for i in self.transpose_forward]))
        seg = None
        data, properties = self.resample_and_normalize_test(data, target_spacing, properties, seg,
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
        ...
        for i in range(num_stages):
            ...
            for j, case in enumerate(list_of_cropped_npz_files):
                ...
                if os.path.exists(os.path.join(output_folder_stage, "%s.npz" % case_identifier)):
                    print(os.path.join(output_folder_stage, "%s.npz" % case_identifier)+'is already saved!!!')
                else:
